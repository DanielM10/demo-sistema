...
import { Api_GetOCModified, Api_OCStatus, api_handleErrors, Api_GetMenus, Api_deleteMenu, Api_deletesubmenu, url_sistema, Api_GetdataSubmenuById, Api_Getdatamenu, Api_SaveDataSubMenu, Api_GetdataSubmenu, Api_SaveDataMenu } from './Api';
class CreateMenu extends React.Component {
  ...
  getsubmenus(id, titulo) {
    this.setState({ "MenuActual": titulo });
    Api_GetdataSubmenu(id)
      .then(api_handleErrors).then(results => {
        return results.json();
      })
      .then(data => {
  ...
  render() {
    return (
      <div className="contenedor-wrapped Fixedpage" id="Wrapper">
        <h1><i className="fas fa-bars"></i> Configuracion de menus</h1>
        <button type="button" className="btn btn-warning float-right" data-toggle="modal" data-target="#exampleModal">
          <i className="fa fa-plus" aria-hidden="true"></i>Nuevo Menu
        </button>
        <br></br>
        <br></br>
        <div className="panel panel-default">
          ...
          <ul className="list-group">
            ...
            {this.state.menus.map(obj => (
              <li className="list-group-item">
                <button type="button" className="btn btn-dark" onClick={() => this.getsubmenus(obj.idMenu, obj.titulo)} value={obj.idMenu}>{obj.titulo} </button>
                <button type="button" onClick={() => this.editarmenu(obj.idMenu)} className="btn btn-success float-right" data-toggle="modal" data-target="#exampleModal">
                  <i className="far fa-edit"></i>Editar
                </button>
                <button type="button" onClick={() => this.eliminarmenu(obj.idMenu, obj.titulo)} className="btn btn-danger float-right">
